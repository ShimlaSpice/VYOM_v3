...
class ScannerEngine:
    ...
    def scan(self, filters: dict | None = None) -> ScanResult:
        ...
        workers = min(max(4, len(symbols) // 20), 24)
        with ThreadPoolExecutor(max_workers=workers) as executor:
            results = list(executor.map(
                lambda symbol: self._scan_symbol(symbol, min_price, max_price, market_change),
                symbols,
            ))
    ...
    def _benchmark_change(self) -> float:
        """Today's % change for the NIFTY benchmark, fetched once per scan."""
        try:
    ...
    def _scan_symbol(
        ...
        latest_price = float(getattr(context, "close", 0.0) or 0.0)
    ...
    # -------------------------------------------------
        ...
        scorecard.add(
            "sma50",
            latest_price > float(indicators.get("sma50", 0.0)),
            "Price above SMA50",
        )
        ...
        scorecard.add(
            "rsi",
            45 <= float(indicators.get("rsi", 0.0)) <= 65,
            f"Healthy RSI ({indicators.get('rsi', 0.0):.2f})",
        )

        scorecard.add(
            "macd",
            float(indicators.get("macd", 0.0)) > float(indicators.get("macd_signal", 0.0)),
            "Bullish MACD",
        )
        ...
        scorecard.add(
            "breakout",
            bool(indicators.get("breakout", False)),
            "Breakout",
        )
        ...
        candidate.breakout = bool(indicators.get("breakout", False))
